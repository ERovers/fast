...
class GromaxProcessing(base):
    ...
    def run(self):
        if self.align_group is None:
            ...
            trjconv_alignment2 = \
                "echo '" + self.align_group + " " + self.align_group + " " + \
                self.output_group + "' | gmx trjconv -f frame0_masses.xtc " + \
                "-o frame0_masses.xtc -s md.tpr -center -fit rot+trans " + \
                "-ur " + self.ur
            if self.index_file is not None:
                trjconv_alignment += " -n " + self.index_file
                trjconv_output_groups += " -n " + self.index_file
                trjconv_alignment2 += " -n " + self.index_file
